#ifndef __CUSTOME_FILE_SINK_H__
#define __CUSTOME_FILE_SINK_H__
...
using custome_file_sink_mt = custome_file_sink<std::mutex>;

// 根据大小和时间生成日志文件
// 参照 rotating_file_sink 和 daily_file_sink 自定义
//（生成的时间也可以修改为任意间隔）
//（还有个需求没完成，就是清理旧文件，按照个数或者时间，即max_files参数目前相关逻辑是没用的）
template <typename Mutex>
class custome_file_sink final : public base_sink<Mutex>
{
public:
    custome_file_sink(filename_t base_filename, size_t max_size, size_t max_files = 0) :
...
using custome_file_sink_mt = custome_file_sink<std::mutex>;
using custome_file_sink_st = custome_file_sink<details::null_mutex>;
} // namespace sinks
...
inline std::shared_ptr<logger> easy_logger_mt(const std::string &logger_name,
                                              ...
                                              size_t max_files = 0)
{
    return Factory::template create<sinks::custome_file_sink_mt>(logger_name, filename, max_size,
                                                                 max_files);
}
...
inline std::shared_ptr<logger> easy_logger_st(const std::string &logger_name,
                                              ...
                                              size_t max_files = 0)
{
    return Factory::template create<sinks::custome_file_sink_st>(logger_name, filename, max_size,
                                                                 max_files);
}
} // namespace spdlog
#endif // __CUSTOME_FILE_SINK_H__