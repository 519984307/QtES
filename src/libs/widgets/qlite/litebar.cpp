    ...
    if (!m_isMaximized && !m_bLeftButtonDbClicked) {
        if (event->globalY() <= 3) {
            ...
            if (m_bWidgetMaximizable) {
                ...
                m_maximizeAction->trigger(); // TODO correct ?
#else
...
#if (QT_VERSION < QT_VERSION_CHECK(5, 0, 0))
    q->emitSigWidgetResizable(false);
#else
...
LiteBar::LiteBar(QWidget *parent) : QObject(parent), d(new LiteBarPrivate())
{
    Q_ASSERT(parent);
